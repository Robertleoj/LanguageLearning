...
fn main() {
    let to_str = |x: &str| -> String {x.to_string()};

    
    println!("{:#?}", Solution::full_justify(["This", "is", "an", "example", "of", "text", "justification."].map(to_str).to_vec(), 16));
    println!("{:#?}", Solution::full_justify(["What","must","be","acknowledgment","shall","be"].map(to_str).to_vec(), 16));
}
